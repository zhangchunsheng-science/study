...
def train(X, y):
    ...
    # 训练模型
    history = model.fit(X_train, y_train, epochs=10000, batch_size=100)
    ...
    print("Mean squared error: %.2f" % mean_squared_error(y_test, y_pred.reshape(-1,)))
    print("Coefficient of determination: %.2f" % r2_score(y_test, y_pred.reshape(-1,)))

    return x, y, X_test, y_pred.reshape(-1,)
